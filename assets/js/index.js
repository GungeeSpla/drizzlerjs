...
import Drizzler from './Drizzler.js';
import Xors from './Xors.js';
import Squid from './Squid.js';
import DraggableBitmap from './DraggableBitmap.js';
...
window.drizzlerjs = (() => {
	...
	const downloadCanvas = () => {
		...
		if (currentCourse.$$trashOpen) {
			trashOpenAlpha = currentCourse.$$trashOpen.alpha;
			trashCloseAlpha = currentCourse.$$trashClose.alpha;
			currentCourse.$$trashOpen.alpha = 0;
			currentCourse.$$trashClose.alpha = 0;
			stage.update();
		}
		// html2canvasで撮影する
		let $base, transform;
	 ...
	 */
	const showOptions = () => {
		$drizzlerjs.querySelector('#drizzlerjs-options-wrapper').style.setProperty('display', 'block');
	 ...
	 */
	const hideOptions = () => {
		$drizzlerjs.querySelector('#drizzlerjs-options-wrapper').style.setProperty('display', 'none');
	}
	...
	const spawnDrizzler = (queueData) => {
		...
		const drizzler = new Drizzler(
			spawnPosition,
			{
				...
				isRTMode: initOptions.isRTMode,
				zmapImageDir: initOptions.zmapImageDir,
				pieceImageDir: initOptions.pieceImageDir,
				weaponImageDir: initOptions.weaponImageDir,
				courseImageDir: initOptions.courseImageDir,
			},
		);
		// push
	...
	const addWeaponPiece = (key) => {
		...
		const bitmap = new DraggableBitmap(
			`${initOptions.weaponImageDir}/${key}.png`,
	...
	const addEnemyPiece = (key) => {
		...
		// ビットマップインスタンスを作成
		const bitmap = new DraggableBitmap(
			`${initOptions.pieceImageDir}/${key}.png`,
			constants.CANVAS_CENTER_X,
			constants.CANVAS_CENTER_Y,
	...
	const toggleConnectMap = () => {
		...
		currentCourse.$$connectMapContainer.set({ alpha });
		// アニメーション
		/*
		utilities.instantTween(
	...
	const selectStartVoronoi = () => {
		...
		if (currentCourse.isSelectingVoronoi) {
			...
			ctx.fillRect(0, 0, initOptions.canvasWidth, initOptions.canvasHeight);
			toggleVoronoi(false);
	...
	const selectCourse = async (course, tide) => {
		...
		const pointer = {
			...
			weaponImageDir: initOptions.weaponImageDir,
			courseImageDir: initOptions.courseImageDir,
			canvasWidth: initOptions.canvasWidth,
			canvasHeight: initOptions.canvasHeight,
		};
		...
		if (courseCaches[key] === undefined) {
			...
			// 使用するzMapの画像データの場所
			const zMapPath = `${initOptions.zmapImageDir}/${course}-${zMapTide}.png`;
			// zMapインスタンスを作成する
			...
			if (tide !== 'high') {
				...
				def = JSON.parse(JSON.stringify(constants.COURSE_DATA[course][tide]));
			} else {
				// 満潮ならば
	...
	const addToolForPieces = (options) => {
		// ラッパーを取得
		const $wrapper = document.getElementById('drizzlerjs-mask-tools');
		// 段の情報
		let row;
		let column;
		let startRow = 0;
		...
		Object.keys(constants.ENEMY_PIECE_DATA).forEach((key, i) => {
			// 画像ファイルの場所
			const path = `${initOptions.pieceImageDir}/${key}.png`;
			// 位置を決定
		...
		Object.keys(constants.WEAPON_DATA).forEach((key, i) => {
			const path = `${initOptions.weaponImageDir}/${key}.png`;
	...
	const initialize = (options) => {
		...
		// 初期化時のオプションを保存しておく
		initOptions = options;
		options.canvasWidth = options.canvasWidth || constants.CANVAS_WIDTH;
		options.canvasHeight = options.canvasHeight || constants.CANVAS_HEIGHT;
		$drizzlerjs = document.getElementById('drizzlerjs-wrapper');
		$forSave = $drizzlerjs.querySelectorAll('.for-save');
		$canvasWrapper = document.getElementById('drizzlerjs-canvas-wrapper');
		loadStorage();
		// キャンバスをまだ作成したことがないなら作成する
		if (!$canvas) {
			// <canvas>要素を作成
			$canvas = utilities.create$canvas(options.canvasWidth, options.canvasHeight);
			// CreateJSのステージを作成
		...
		$drizzlerjs.style.setProperty('width', options.canvasWidth + 'px');
		$drizzlerjs.style.setProperty('height', options.canvasHeight + 'px');
		...
		// ツールの切り替え
		if (options.isRTMode) {
			document.getElementById('drizzlerjs-rtmode-tools')
				.style.setProperty('display', 'block');
			document.getElementById('drizzlerjs-not-rtmode-tools')
		...
		if (initOptions.lang === 'en') {
			const $forTranslations = $drizzlerjs.querySelectorAll('.for-translation');
			Array.prototype.forEach.call($forTranslations, ($forTranslation) => {
				$forTranslation.textContent = $forTranslation.getAttribute('en-text');
			});
		}
		const $stageSelect = $drizzlerjs.querySelector('#drizzlerjs-course-select');
		if ($stageSelect) {
	...
	const finalize = () => {
		...
		// 追加したDOM要素を削除
		utilities.empty$element('drizzlerjs-canvas-wrapper');
		utilities.empty$element('drizzlerjs-mask-tools');
		// 変数を初期化
		initializeVariables();
