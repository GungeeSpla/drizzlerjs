import * as constants from './constants.old.js';
...
(() => {
	...
	const createPathDataFromImage = (image, tryCount = 0) => new Promise((resolve) => {
		...
		// 頂点群をサーチ
		const vertexes = searchVertexes(imagedata);
		// 結果を返す
		resolve({
			vertexes,
		});
	...
	const Vtx = function Vtx(id, pixels, arounds, type) {
		...
		this.radius = Math.round((this.xMax - this.xMin) / 2);
		this.x = Math.round(xSum / num);
		this.y = Math.round(ySum / num);
		this.connects = [];
	};
	...
	Vtx.prototype.drawVertex = function drawVertex(color, size, noText) {
		if (! size) size = 6;
		const str = (this.gusherIndex !== undefined) ? `[${String.fromCharCode(('A').charCodeAt() + this.gusherIndex)}]` : this.id;
		if (! color) {
					...
					if (pixels.length !== 0) {
						...
						switch (type) {
							...
							case COLOR_TYPE_GUSHER_VERTEX:
								const r = getBlueValue(imagedata, x, y);
	...
	/** getBlueValue(imagedata, x, y)
	 */
	const getBlueValue = (imagedata, x, y) => {
		const i = 4 * (Math.floor(x) + Math.floor(y) * imagedata.width);
		const b = imagedata.data[i + 2];
		return b;
	};
	...
	const logPathData = (pathData) => {
		pathData.vertexes.sort(function(a, b) {
			return a.gusherIndex - b.gusherIndex;
		});
		const arr = [];
		pathData.vertexes.forEach((vertex) => {
			arr.push([
				vertex.x,
				vertex.y,
			]);
	...
	const showSamplePathData = async (options) => {
		...
		const $$image = new createjs.Bitmap(`${options.courseImageDir}/${options.stage}-${options.tide}.png`);
		$$image.image.onload = () => {
			stage.update();
		};
		stage.addChild($$image);
		
		const key = `${options.stage}-${options.tide}`;
		const def = constants.COURSE_DATA[options.stage][options.tide];
		
		
		Object.keys(constants.COURSE_DATA).forEach((stage) => {
			['normal', 'high', 'low'].forEach((tide) => {
				const def = constants.COURSE_DATA[stage][tide];
				if (! def.x) def.x = constants.COURSE_DATA[stage]['normal'].x;
				if (! def.y) def.y = constants.COURSE_DATA[stage]['normal'].y;
				if (! def.scale) def.scale = constants.COURSE_DATA[stage]['normal'].scale;
				if (! def.rotation) def.rotation = constants.COURSE_DATA[stage]['normal'].rotation;
				const f = (dot) => {
					const x = dot[0];
					const y = dot[1];
					const vecx = x - (640 / 2) + (640 / 2 - def.x);
					const vecy = y - (960 / 2) + (960 / 2 - def.y);
					const rotvec = utilities.getRotatedVector({
						x: vecx,       
						y: vecy,
					}, -def.rotation);
					const x2 = rotvec.x / def.scale;
					const y2 = rotvec.y / def.scale;
					dot[0] = Math.round(1000 + x2);
					dot[1] = Math.round(1000 + y2);
				};
				if ('parks' in def) {
				...
				if ('pushedParks' in def) {
					Object.keys(def.pushedParks).forEach((key) => {
						const dot = def.pushedParks[key];
						f(dot);
					});
				}
				if ('homes' in def) {
		...
		console.log(JSON.stringify(constants.COURSE_DATA));
...
/*
		...
		pathData = await createPathDataFromImage(`${options.coursePathImageDir}/${options.stage}-${options.tide}.png`);
		logPathData(pathData);

		pathData.vertexes.forEach((vtx) => {
